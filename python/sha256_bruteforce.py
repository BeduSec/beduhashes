...
def print_banner():
    banner = f"""
\n\u2800\u2800SHA-256 DECRYPTER PRO - by BeduSec Security Systems
    """
...
def main():
    ...
    target_hash = input("\033[95m[?] Enter SHA-256 Hash: \033[0m").strip().lower()
    if len(target_hash) != 64 or not all(c in '0123456789abcdef' for c in target_hash):
        print("\033[91m[!] Invalid SHA-256 hash format (must be 64 characters).\033[0m")
        return
